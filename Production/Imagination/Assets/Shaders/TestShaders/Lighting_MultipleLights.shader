      ...
      Pass {      
         ...
         {          
            ...
            {    
               ...
               float3 vertexToLightSource = 
                  float3(lightPosition.xyz - output.posWorld.xyz);        
               float3 lightDirection = normalize(vertexToLightSource.xyz);
               float squaredDistance = 
                  dot(vertexToLightSource, vertexToLightSource);
               float attenuation = 1.0 / (1.0 + 
                  unity_4LightAtten0[index] * squaredDistance);
               float3 diffuseReflection =  
                  attenuation * float3(unity_LightColor[index].xyz) 
                  * float3(_Color.xyz) * max(0.0, 
         ...
         float4 frag(vertexOutput input) : COLOR
         {
            ...
            float3 viewDirection = normalize(
               _WorldSpaceCameraPos.xyz - float3(input.posWorld.xyz));
